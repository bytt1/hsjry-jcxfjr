...
public class IncomingPartsPort {
    ...
    private static final String CONFIRM_LOAN_URL = DOMAIN + "/v1/credit/commitLoanApply";
    private static final String QUERY_CONTRACT_MODULE_URL = DOMAIN + "/v1/credit/preViewContract";
    private static final String SIGN_LOAN_CONTRACT_URL = DOMAIN + "/v1/credit/signLoanContract";
    ...
    //登记婚姻信息
    public static JSONObject registerMarriageInfo(String creditApplyId,String certNo,String spousePhone,String userId) throws IOException {
        return Okhttp.analysisToJson(Okhttp.doPost(REG_MARRIAGE_INFO_URL,
                Okhttp.requestBody(IncomingPartsData.regMarriageInfo(creditApplyId,certNo,spousePhone,userId)).toJSONString()));
    }
    ...
    //添加关联人
    public static JSONObject addCorrelationPerson(String mainCrtAlyId,String certNo,String uid) throws IOException {
        JSONObject reqData = IncomingPartsData.addCorrelationPerson(mainCrtAlyId,certNo,uid);
    ...
    public static JSONObject loanChoiceDeadlineQuery(String mainApplyId,String creditApplyId) throws IOException {
        return Okhttp.analysisToJson(Okhttp.doPost(CRT_CHOICE_DEADLINE_QUERY_URL,
                Okhttp.requestBody(IncomingPartsData.loanSelectableDeadlineQuery(mainApplyId,creditApplyId))
                        .toJSONString()));
    }

    //确认贷款
    public static JSONObject confirmLoan(String maiCreditApplyId,String creditApplyId,String bankCard,String userId) throws IOException {
        return Okhttp.analysisToJson(Okhttp.doPost(CONFIRM_LOAN_URL,
                Okhttp.requestBody(IncomingPartsData.confirmLoan(maiCreditApplyId,creditApplyId,bankCard,userId))
                        .toJSONString()));
    }

    //合同模板查询
    public static JSONObject queryContractModule(String loanApplyId) throws IOException {
        return Okhttp.analysisToJson(Okhttp.doPost(QUERY_CONTRACT_MODULE_URL,
                Okhttp.requestBody(IncomingPartsData.contractModule(loanApplyId))
                        .toJSONString()));
    }


    public static JSONObject signLoanContract(String userId,String loanApplyId,
                                              String tradePwd,JSONObject preViewContract) throws IOException {
        return Okhttp.analysisToJson(Okhttp.doPost(SIGN_LOAN_CONTRACT_URL,
                Okhttp.requestBody(IncomingPartsData.signLoanContract(userId,loanApplyId,
                        tradePwd,preViewContract)).toJSONString()));
    }
...
//
//        staySupplementAccessoryListQuery("");
...
//        PublicFunc.secRiskMngCallback("4c21733fa7a34627b5416d390fdb7c15");
        String s = "{\"pmtAddress\":\"京仁秘涕绣衫\",\"certType\":\"0\",\"attachments\":[{\"fileName\":\"身份证正面\",\"fileKind\":1,\"fileUrl\":\"http://jccfc-dev.ks3-cn-beijing.ksyun.com/xdgl/ydhb/yw/wyrc/20200101/2020010208.jpg\"},{\"fileName\":\"身份证反面\",\"fileKind\":2,\"fileUrl\":\"http://jccfc-dev.ks3-cn-beijing.ksyun.com/xdgl/ydhb/yw/wyrc/20200101/2020010207.jpg\"}],\"gender\":1,\"pmtAddrProvinceCode\":\"230000\",\"nation\":\"汉\",\"dueDate\":\"20260915\",\"signOrgan\":\"成都市公安局\",\"pmtAddrAreaCode\":\"231025\",\"userName\":\"韩祥\",\"userId\":\"000UC020000853788\",\"authMode\":0,\"certNo\":\"422223195902224918\",\"issueDate\":\"20060915\",\"pmtAddrCityCode\":\"231000\",\"certificateCheck\":\"1\"}";

        supplementInfoCommit(PublicFunc.getProductID(),"000CA2020050000000543",JSON.parseObject(s),
                PublicFunc.getUserId("13711775766"),"main");
//        writerPro(realName,"realName",true);

//        correlationInfoQuery("000CA2020040000077772");

//        mortgageCorrelationInfoManagementQuery("000CA2020040000077256");

//        confirmLoan("000CA2020050000000051","000CA2020050000000053",
//                "6217004220042538946",PublicFunc.getUserId("13750717265"));
//        String loanApplyId_query_sql = "select loan_apply_id from credit_loan_apply where" +
//                " credit_apply_id = \"000CA2020050000000051\"";
//
//        String loanApplyId = SqlUtils.select(loanApplyId_query_sql);
//        JSONObject res = queryContractModule(loanApplyId);
//
//        JSONObject contractModule = res.getJSONObject("body").getJSONArray("contractList").getJSONObject(0);
//
//        signLoanContract("",loanApplyId,"",contractModule);
