...
public class AccountBindingData {
    ...
    //绑定银行卡
    public static JSONObject bankCardBinding(String account,String code){
        JSONObject bind = publicField;

        bind.put("authMode",2); // 必传  绑定模式 0仅鉴权 1仅登记 2鉴权并登记
        bind.put("account",account); //  必传 银行卡号
        bind.put("accountName","吴洲"); // 账户名
        bind.put("certNo","422223196902224906"); //
        bind.put("accountKind","0"); // 银行卡类别
        bind.put("mobileNo","13417118858"); // 预留手机号
        bind.put("unionBankId","102651086203"); // 条件必传（个人银行卡的开户银行支付联行行号
                                    //绑定模式为仅登记或鉴权并登记且非本行账户不能为空）
        bind.put("unionBankName","中国工商银行"); // 支付行名
        bind.put("tradePassword",""); //  条件必传（若需要校验客户交易密码则不能为空）   交易密码
        bind.put("smsCode",code); // 条件必传（若需要校验客户短信验证码则
                                // 绑定模式为仅鉴权或鉴权并登记时不能为空）
        return bind;
    }
